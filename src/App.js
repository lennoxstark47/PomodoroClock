...
class App extends React.Component {
	constructor() {
		...
		this.intervalTypes = Object.keys(
	...
	handleChange(event) {
		...
		if (
			intervals[intervalType] >= 1 &&
			intervals[intervalType] <= 60
		) {
	...
	pause() {
		...
		this.setState({
			isPaused: true,
			pausedTime: this.state.time.split(':'),
		});
	}
